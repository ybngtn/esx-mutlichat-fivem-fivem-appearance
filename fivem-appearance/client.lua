...
CreateThread(function()
    ...
    for i=1, #Config.EjSurgery do
        if Config.EjSurgery[i].blip then
            CreateBlip(Config.EjSurgery[i].coords, 499, 66, Config.Translation.Blip.EjSurgery, 0.7  )
        end
    end
end)

CreateThread(function()
        if Config.UseQorOXTarget then
            --print('fivem drawer disabled')
        else
            while true do
        local sleep = 1500
        local playerCoords, inClothingShop, inBarberShop, inEjSurgery, inTattooShop, currentZone = GetEntityCoords(PlayerPedId()), false, false, false, nil
        for i=1, #Config.ClothingShops do
        ...
        for i=1, #Config.EjSurgery do
            local dist = #(playerCoords - Config.EjSurgery[i].coords)
            if dist <= Config.Distance then
                sleep = 0
                if dist <= Config.EjSurgery[i].distance then
                    inEjSurgery, currentZone = true, i
                end
            end
        end
        ...
        if (inEjSurgery and not hasAlreadyEnteredMarker) or (inEjSurgery and LastZone ~= currentZone) then
            hasAlreadyEnteredMarker, LastZone = true, currentZone
            CurrentAction = 'Surgery'
            lib.showTextUI(Config.Translation.Menu.EjSurgery)
        end
        ...
        if not inClothingShop and not inBarberShop and not inTattooShop and not inEjSurgery and hasAlreadyEnteredMarker then
    ...
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlPressed(1, 38) then
                ...
                elseif CurrentAction == 'Surgery' then
                    local config = {
                        ped = false,
                        headBlend = true,
                        faceFeatures = true,
...
RegisterNetEvent('fivem-appearance:EjSurgery')
AddEventHandler('fivem-appearance:EjSurgery', function()
	local config = {
		ped = true,
		headBlend = true,
		faceFeatures = true,
	...
	exports['fivem-appearance']:startPlayerCustomization(function (appearance)
		if (appearance) then
			...
			ESX.SetPlayerData('ped', PlayerPedId())
			TriggerEvent('esx:restoreLoadout')
            TriggerServerEvent('ej:removesurgerycard')
		else
			ESX.SetPlayerData('ped', PlayerPedId())
			TriggerEvent('esx:restoreLoadout')
		end
...
exports.ox_target:addSphereZone({
    coords = vec3(429.7724, -797.7897, 29.4911),
    radius = 1,
    debug = false,
    options = {
        {
